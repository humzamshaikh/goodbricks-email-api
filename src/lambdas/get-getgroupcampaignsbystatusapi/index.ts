...
interface GetGroupCampaignsByStatusRequest {
  cognitoId: string;
  groupId: string;
  status: string;
}
...
interface GroupStatusCampaignsResponse {
  success: boolean;
...
const handlerLogic = async (event: ApiGatewayEventLike): Promise<GroupStatusCampaignsResponse> => {
  try {
    // Parse request body to get cognitoId, groupId, and status
    ...
    console.log(`Getting ${status} campaigns for group: ${groupId} (user: ${cognitoId})`);
    ...
    console.log(`Found ${campaigns.length} ${status} campaigns for group: ${groupId} (user: ${cognitoId})`);
    ...
    return {
      ...
      message: `Found ${campaigns.length} ${status} campaigns for group: ${groupId}`
    };

  } catch (error) {
    console.error('Error getting group campaigns by status:', error);
    if (error instanceof HttpError) {
      throw error;
    }
    throw new HttpError(500, `Failed to get group campaigns by status: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
};
